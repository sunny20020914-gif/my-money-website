 ...
 *
 * placement は、そのブロックをランキング一覧の
 * 「前」に置くか「後ろ」に置くかの指定。
 *
 * 【なぜ分けるか】
 * リード文3ブロックをすべて冒頭に置くと、スマホでは約830pxを占め、
 * 1位のカードが現れるまで2.3画面ぶんスクロールが必要だった。
 * 「初任給ランキング」で来た人が、ランキングを見るまでに2回スクロールする状態。
 *
 * ただし全部を下に送るのも正しくない。Googleはレンダリング後の
 * 視覚的な位置を見ており、本文冒頭の記述には相対的な重みがある。
 * また「初任給の相場を知りたい」で来た層への回答も冒頭に要る。
 *
 * そこで
 *   before … 相場（＝検索意図への直接の回答。短い）
 *   after  … 業界の傾向・読み方の注意（＝一覧を見たあとに読む話）
 * と役割で分ける。文章はどちらもHTMLに残るため、
 * 下に置いてもインデックスはされる（Googleは初期表示で
 * 隠れている内容も通常のコンテンツとして扱うと明言している）。
 */
export type LeadPlacement = "before" | "after"
...
export function buildRankingLead(opts: {
  ...
  // ④【削除済み】かつてここに「額面だけで比べて大丈夫？」という段落があった。
  // 固定残業代や住宅手当が含まれるので同じ30万円でも条件は違う、という内容。
  //
  // だがランキング下部の解説セクションに
  // 「求人票の初任給を正しく読み解く3つのポイント」があり、
  // 固定残業代・諸手当・昇給カーブを具体例つきで詳しく説明している。
  // リード文を一覧の後ろに移した結果、この2つが隣り合うことになり、
  // 短い要約版とその完全版が並ぶ状態になってしまった。
  //
  // 内容の薄いほうを残す理由がないため、こちらを削除する。
